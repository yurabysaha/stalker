  ...
  def buy_on_market
    ...
    if @item.in_market == true
      if current_user.user_profile.money >= @item.price
        ...
        seller.money = seller.money + @item.price
        seller.q_sold_on = seller.q_sold_on + @item.price
        seller.weight = seller.weight - @item.add_weight
        seller.save
        buyer = current_user.user_profile
        buyer.money= buyer.money - @item.price
        buyer.weight = buyer.weight + @item.add_weight
        buyer.save
